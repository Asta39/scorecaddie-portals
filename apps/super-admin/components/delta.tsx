...
function DeltaIcon({
	...
	if (!resolvedValue || resolvedValue === 0) {
		return shell(
			<MinusIcon {...slotProps} className={mergedClassName} {...(props as any)} />
		);
	}
	...
	if (resolvedValue > 0) {
		if (variant === "trend") {
			return shell(
				<TrendingUpIcon {...slotProps} className={mergedClassName} {...(props as any)} />
			);
		}

		if (variant === "arrow") {
			return shell(
				<ArrowUpIcon {...slotProps} className={mergedClassName} {...(props as any)} />
			);
		}

		return shell(
			<ChevronUpIcon {...slotProps} className={mergedClassName} {...(props as any)} />
		);
	}

	if (variant === "trend") {
		return shell(
			<TrendingDownIcon {...slotProps} className={mergedClassName} {...(props as any)} />
		);
	}

	if (variant === "arrow") {
		return shell(
			<ArrowDownIcon {...slotProps} className={mergedClassName} {...(props as any)} />
		);
	}

	return shell(
		<ChevronDownIcon {...slotProps} className={mergedClassName} {...(props as any)} />
	);
}
