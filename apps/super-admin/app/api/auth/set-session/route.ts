 ...
 * very next request (confirmed via Vercel runtime logs on club-admin, same
 * architecture: middleware logged "Auth session missing!" repeatedly).
 * Doing it here instead means middleware and every other server-side read
 * sees a properly-issued cookie from the start.
 */
