      ...
      if (currentClubId && params?.id) {
        ...
        if (data && !error) {
          setFormData({
            ...
            entry_deadline: data.entry_deadline ? new Date(data.entry_deadline).toISOString().slice(0, 16) : '',
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          start_date: formData.is_template ? '1970-01-01' : formData.start_date,
      ...
      router.push(`/competitions/${params.id as string}`)
    } catch (err: any) {
    ...
    <div className="portal-content max-w-3xl">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold" style={{ color: 'var(--color-text)' }}>
              {formData.is_template ? 'Edit Recurring Template' : 'Edit Competition'}
            </h1>
            <p className="text-sm mt-0.5" style={{ color: 'var(--color-text-muted)' }}>
              Update details for your {formData.is_template ? 'competition template' : 'formal competition'}
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex gap-4 pt-4">
          <button 
            type="button" 
            onClick={() => router.push(`/competitions/${params?.id as string}`)}
