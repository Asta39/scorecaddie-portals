...
export default function TeeTimesPage() {
  ...
  const saveSettings = async () => {
    ...
    const { error } = await supabase.from('course_tee_time_settings').upsert({
      course_id: courseId,
  ...
  const exportTeeSheetPdf = () => {
    ...
    const brand: [number, number, number] = [27, 122, 78] // emerald700, matches app brand
    const brandLight: [number, number, number] = [232, 245, 237]
      ...
      rows.push([
        {
          ...
          colSpan: 5,
          styles: { fillColor: brandLight, fontStyle: 'bold', textColor: [20, 60, 40], fontSize: 11 },
        },
      ])
    ...
    if (rows.length === 0) {
      doc.setFontSize(11)
      doc.text('No bookings for this date.', 14, 42)
    } else {
      autoTable(doc, {
        startY: 40,
        head: [['#', 'Player', 'Type', 'Handicap', 'Present']],
        body: rows as any,
        theme: 'grid',
        ...
        didDrawCell: (data) => {
          // Draw a checkbox square in the blank "Present" column for the starter to tick.
          if (data.column.index === 4 && data.row.section === 'body' && data.cell.raw === '') {
            const boxSize = 5
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text('Generated by ScoreCaddie', 14, doc.internal.pageSize.getHeight() - 8)
  ...
  const blockTimeSlot = async (timeSlot: string) => {
    ...
    const { error } = await supabase.from('course_blocks').insert({
      course_id: courseId,
                    ...
                    return (
                      <Fragment key={index}>
                        <tr className={slot.is_blocked ? 'bg-red-50/50 cursor-pointer' : 'cursor-pointer hover:bg-gray-50'}
                            onClick={() => setExpandedRow(isExpanded ? null : slot.time_slot)}>
                        ...
                        {isExpanded && players.length === 0 && (
                          <tr className="bg-gray-50/50">
                            <td colSpan={4} className="p-4 border-b border-light text-center text-sm text-text-muted">
